/*
  **Reminder for me: gcc ICT-CMS.c -o ICT-CMS pdcurses.lib <-- Don't forget!!!
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main()
{
    ...
    //Top Banner
    width = COLS-2;
    posi = (width-strlen(toptext))/2;
    topwin = newwin(3, width, 1, 1);
    for(i = 0; i < width; i++) wprintw(topwin,"=");
    mvwprintw(topwin,1,posi, "%s\n", toptext);
    for(i = 0; i < width; i++) wprintw(topwin,"=");
    wrefresh(topwin);
...
void menu_Main()
{
    WINDOW *menu_choice, *menu_bottom;
...
void student_menu(int user_index)
{
    WINDOW *student_choice, *student_bottom;
    ...
    switch (choice)
    {
        case 1:win_delete(student_choice); edit_info(1, user_index);  break;
        case 2:win_delete(student_choice); show_all(1, user_index);   break;
        case 3:win_delete(student_choice); searchinfo(1, user_index); break;
        case 4:
            wclear(student_choice);
            mvwprintw(student_choice,LINES/2, (COLS - strlen(bye))/2, "%s", bye);
            wgetch(student_choice);
...
void admin_menu(int user_index)
{
    WINDOW *admin_choice, *admin_bottom;
    int i, loop_con=1, hilight = 1, window_width, choice_height, choice, wch;
    char *login_choices[] = {
        "Edit User's Contact",
        "Show all Contacts",
    ...
    switch (choice)
    {
        case 1:win_delete(admin_choice); edit_info(0, user_index);  break;
        case 2:win_delete(admin_choice); show_all(0, user_index);   break;
        case 3:win_delete(admin_choice); searchinfo(0, user_index); break;
        case 4:win_delete(admin_choice); delete_info(user_index); break;
        case 5:
            wclear(admin_choice);
            mvwprintw(admin_choice,LINES/2, (COLS - strlen(bye))/2, "%s", bye);
            wgetch(admin_choice);
...
void searchinfo(int role, int user_index)
{
    ...
    char input[50], name[50], er[100] = "There is no person Name '", er2[]="' in the system.", *s;
    ...
    while (f==0)
    {
        ...
        for (i = 0; i < total_user; i++)
        {
            strcpy(name, user_all[i].firstName);
            strlwr(name);
            strlwr(input);
            if (s = strstr(name, input))
            {
                f++;
                index[j] = i;
                j++;
